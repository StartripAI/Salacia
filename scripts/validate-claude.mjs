...
async function main() {
  ...
  if (!process.env.ANTHROPIC_AUTH_TOKEN) {
    console.log(
      JSON.stringify({
        vote: 'reject',
        summary: 'ANTHROPIC_AUTH_TOKEN is required',
  ...
  try {
    const { stdout, stderr } = await execFileAsync('claude', ['-p', '--model', model, prompt], {
      env,
    ...
    if (!payload.summary) payload.summary = 'Claude response parsed';
